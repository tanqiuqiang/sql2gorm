...
func makeCode(stmt *ast.CreateTableStmt, opt options) (string, []string, error) {
	...
	isPrimaryKey := make(map[string]bool)  //主键
	isIndex := make(map[string]string)     //普通索引
	isUniqindex := make(map[string]string) //唯一索引
	for _, con := range stmt.Constraints {
		if con.Tp == ast.ConstraintPrimaryKey {
			isPrimaryKey[con.Keys[0].Column.String()] = true
		} else if con.Tp == ast.ConstraintIndex {
			isIndex[con.Keys[0].Column.String()] = con.Name
		} else if con.Tp == ast.ConstraintUniq || con.Tp == ast.ConstraintUniqKey || con.Tp == ast.ConstraintUniqIndex {
			isUniqindex[con.Keys[0].Column.String()] = con.Name
		}
	}
	...
	for _, col := range stmt.Cols {
		...
		if isIndex[colName] != "" {
			gormTag.WriteString(";index:" + isIndex[colName])
		}
		if isUniqindex[colName] != "" {
			gormTag.WriteString(";uniqueIndex:" + isUniqindex[colName])
		}
		isNotNull := false
		canNull := false
		...
		if !canNull {
			nullStyle = NullDisable //todo :time.Time bug
		}
		nullStyle = NullDisable //todo :time.Time bug
